...
export async function createImports(config) {
  ...
  function version(job) { return digest(JSON.stringify([job.id,job.originalHash,job.cardHash,...outputPaths(job),...(job.files?[job.files,job.parser,job.warnings]:[])])); }
  async function cardFor(job) {
    ...
    const fields = {
      ...
      '- 未读取范围：':'- 未读取范围：'+(job.files?job.warnings.join('；'):'文档中的附件与外部链接未读取；未开展知识提炼。'),
      '- 来源：':job.files?`- 来源：[上传的原件](./原文.${job.format})；[[./解析正文|自动解析正文]]`:'- 来源：[[./原文|上传的 Markdown 原件]]'
    };
  ...
  async function create(input) {
    return locked(async () => {
      ...
      if (!/\.(?:md|docx|pdf)$/i.test(name) || /[/\\]/.test(name)) throw failure(400, '只接受 Markdown、DOCX 或 PDF；旧 DOC 暂不支持');
      const format = path.extname(name).slice(1).toLowerCase(), maximum = format==='md'?limit:16*1024*1024;
      ...
      const parsed = format==='md'?null:await parseFile(bytes,format,config.imports.python);
      ...
      if (parsed) artifacts.push({name:'content.md',bytes:Buffer.from(`[完整原件](./原文.${format})\n\n`+parsed.markdown)},...parsed.assets);
  ...
  async function preview(id) {
    ...
    return {...job,category:categoryFor(job.target),outputs:entries.map(({path,sha256})=>({path,sha256})),original:job.files?await fs.readFile(path.join(folder,'content.md'),'utf8'):original.toString('utf8'),card:card.toString('utf8'),html,
      originalUrl:`/api/imports/${id}/file?path=${encodeURIComponent('原文.'+(job.format||'md'))}`,warnings:[...(job.warnings||['仅解析本次 Markdown 文字；附件与外链未读取。']),'资料归档不代表知识结论已审核。',...originalView.links.filter(link=>!['resolved','external'].includes(link.status)).map(link=>'未读取链接：'+link.raw)], resourceRoot};
  }
