...
本流程通过安装时确认启用，仅在业务分析、业务开发或明确要求查询知识库时，通过两个子 Agent 完成知识查询与候选评审，不依赖 AGENTS.md 调度。
1. 主 Agent 先根据用户请求和当前任务判断适用范围：仅业务分析、业务开发或用户明确要求查询知识库时调用 knowledge_retriever；普通问答、聊天、文档润色和非业务配置维护直接跳过查询与评审，不启动知识子 Agent。“继续”等简短消息沿用当前任务范围，不因消息本身重新触发；已有查询覆盖且任务范围未变时复用结果。需要查询时传入用户原话、必要上下文、当前任务与工作项目；由子 Agent 判断是否需要实际检索。取消、停止、撤销授权优先立即执行，不等待查询。
...
4. 仅对第 1 条适用范围内的任务，在形成可交付结果后调用 knowledge_reviewer；跳过知识流程的普通问答等任务不调用。提供实际结论、范围、证据位置与版本、已有验证记录、查询证据包、未解决项及本任务已有候选/拒绝反馈；还要提供与结果有关的关键取舍及理由、失败尝试及结果、工具/技能的具体作用、成功步骤与验证方式，并绑定可定位证据。只传相关过程摘要，不传完整会话或凭据，不编造缺失过程。最终答案重复时，也交由评审判断过程是否有新增经验。让它独立比较旧知识；不要预先写好一篇知识要求它通过。任务尚未完成不评审；已评审且没有实质变化不重复评审。
